...
public class Player2 implements IPlayer {
	 ...
	 */
	public boolean oneDirectionJudge(int x, int y, int color, int xStep, int yStep,boolean modifyOrNot, int[][] board){
		int xMv,yMv;
		...
		while(true){
			if(!isInBounds(xMv,yMv)){//如果越界则停止，此位置不合法
				...
				if(xMv==x+xStep && yMv==y+yStep)  break; //如果是第一个就同色则停止，不合法	
					...
					if(modifyOrNot) lineModify(x+xStep,y+yStep,xMv-xStep,yMv-yStep,color,board);
					return true;
				}
			}
			xMv += xStep; yMv += yStep;
	...
	public String botJudge1(){
		String myMessage = "NO";
		boolean flag = true;
		for(int x=0; x<8 && flag; x++){//这里采用最简单策略，即遍历，遇到第一个合法落棋位置则输出
			for(int y=0; y<8 && flag; y++){
				if(this.pieceLegalJudge(x, y, myColor, true)){//当前位置合法
					flag = false;
